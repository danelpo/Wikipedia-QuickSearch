...
function retriveSearch(searchWord) {
    ...
    const articleLang = new RegExp('([^\x01-\x7E])+', 'g').exec(article) ? 'ja' : 'en';

    const proxyURL = 'https://cors-anywhere.herokuapp.com/';
    const url = `https://${articleLang}.wikipedia.org/w/api.php?action=query&prop=extracts&format=json&exintro=&explaintext=&titles=${article}`;
...
function makePopup(articleExtract, articleTitle, articleLang) {
    ...
    imageLink.href = `https://${articleLang}.wikipedia.org`;
    ...
    if (articleExtract.length == 0) {
        article.innerHTML = articleLang == 'ja' ? 'ウィキペディアには現在この名前の項目はありません。' : 'Wikipedia does not have an article with this exact name';
    } else {
    ...
    const link = `https://${articleLang}.wikipedia.org/wiki/${articleTitle}`;

    let articleLink = document.createElement('a');
    articleLink.href = link;
    articleLink.title = link;
    articleLink.innerHTML = articleLang == 'ja' ? '続きを読む' : 'Read more';
